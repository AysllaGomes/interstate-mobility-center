...
export class ViagemStrangerService extends ViagemParceiroAbstract {
  public async retornaCotacao(req: any, userIdParceiro: number, token: string): Promise<any | undefined> {
    try {
      logger.info("Buscando cotacoes da Stranger");
      ...
      logger.error(`ERRO na busca de cotações da Stranger: ${error}`);
  ...
  public async solicitaViagem(solicitacaoViagem: ISolicitacaoViagemParceiro, token: string): Promise<any> {
    try {
      logger.debug("Solicitando viagem na Stranger");

      const endpoint = `${environment.parceiros.host}/rides/`;

      logger.debug("Setando dados no formato do json da Stranger");
  ...
  public async pegaStatusAdapter(viagem: IInterfaceViagem, token: string): Promise<IStatusViagemParceiro> {
    try {
      logger.info("Buscando status da viagem no parceiro Stranger");

      const jsonResult: AxiosResponse | undefined = await ViagemStrangerService.requestStatus(viagem, token);
      ...
      logger.error(`ERRO na busca de status da viagem no parceiro Stranger: ${error}`);
  ...
  private static async requestStatus(viagem: IInterfaceViagem, token: string): Promise<AxiosResponse | undefined> {
    ...
    try {
      ...
      logger.error(`
        ...
        <'ERRO'>
          message: ERRO ao retornar status de viagem do parceiro Stranger...
        Parâmetros da requisição:
          ID VIAGEM: ${idViagem}
  ...
  public async cancelarViagem(idViagemNoParceiro: string, token: string): Promise<boolean> {
    try {
      logger.info("Cancelando viagem na Stranger...");
      ...
      logger.error(`ERRO ao cancelar viagem no parceiro Stranger: ${err}`);

      throw new ErroExterno(...ERRO_EXTERNO_DO_PARCEIRO).formatMessage("Stranger");
    }
  }
}
